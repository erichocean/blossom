...
/** @scope SC.View.prototype */ {
  ...
  */
  ...
  /**
    ...
    the `ignoreVisibility` parameter.
  ...
  */
  ...
  // ..........................................................
  // KEY VIEW LOOP SUPPORT (TAB AND SHIFT-TAB)
  //
  ...
  */
  nextValidKeyView: function() {
    ...
    if(!ret) ret = rootView._sc_computeNextValidKeyView(this, seen);

    if(SC.TABBING_ONLY_INSIDE_DOCUMENT && !ret) {
      ret = rootView._sc_computeNextValidKeyView(rootView, seen);
    }

    return ret ;
  }.property('nextKeyView'),

  _sc_computeNextValidKeyView: function(currentView, seen) {
    ...
    if (!ret) {
      ...
      for(i=0, childLen = children.length; i<childLen; i++){
        ...
        if(child.get('isVisibleInWindow') && child.get('isVisible')){
          ret = child._sc_computeNextValidKeyView(currentView, seen);
        }
        if (ret) return ret;
      }
      ret = null;
  ...
  */
  previousValidKeyView: function() {
    ...
    if(!ret) ret = rootView._sc_computePreviousValidKeyView(this, seen);
    return ret ;
  }.property('previousKeyView'),

  _sc_computePreviousValidKeyView: function(currentView, seen) {
    ...
    if (!ret) {
      ...
      for(i=children.length-1; 0<=i; i--){
        child = children[i];
        if(child.get('isVisibleInWindow') && child.get('isVisible')){
          ret = child._sc_computePreviousValidKeyView(currentView, seen);
        }
        if (ret) return ret;
      }
      ret = null;
  ...
  */
  ...
  /**
    ...
    @returns true if the contextmenu can show up
  */
...
/** @scope SC.View.prototype */ { // Monkey patch for now.
  ...
  /**
    Optional background color.  Will be applied to the view's root layer if
    set.
  ...
  */
  ...
  /** @property
    true if the view is currently first responder and the surface the view 
  ...
  */
  recomputeIsVisibleInWindow: function(parentViewIsVisible) {
    ...
    if (previous !== current) {
      ...
      // resign it.  This more appropriately belongs in an
      // 'isVisibleInWindow' observer or some such helper method because
      // this work is not strictly related to recomputing the visibility, but
      // view performance is critical, so avoiding the extra observer is
      // worthwhile.
      if (!current && this.get('isFirstResponder')) {
        this.resignFirstResponder();
      }
    }
  },
  ...
  /**
    Insert the view into the the receiver's childViews array.

    The view will be added to the childViews array before the beforeView.  If
    beforeView is null, then the view will be added to the end of the array.
    This will also add the view's root layer to the receiver's
    containerLayer as a sublayer.

    If the specified view already belongs to another parent, it will be
    removed from that view first, and it's layer will likewise be removed.
  ...
  */
  ...
  /**
    Replace the oldView with the specified view in the receivers childViews
    array. This will also replace the layer of the oldView with the layer
    of the new view in the receivers root layer.

    If the specified view already belongs to another parent, it will be
    removed from that parent first.

    @param view {SC.View} the view to insert
    @param oldView {SC.View} the view to replace
  */
  ...
  */
  ...
  /**
    ...
    hierarchy.  This method will update the view's layer so that it is a 
    sublayer of the receiver's parentView's layer.  It will also instruct 
    the pane this view is part of to re-composite during the next render 
    loop.
  */
  ...
  */
  ...
  /**
    The tool tip string that should be displayed over the view when the user 
    hovers for a moment.
  ...
  */
  ...
  /**
    ...
    child views). This will remove the view from any parent view, and destroy 
    it's layer and any sublayers.

    You should not remove yourself from your parent view during this method 
    when `skipRemoveFromParent` is true.

    @param {Boolean} skipRemoveFromParent
  */
  destroy: function(skipRemoveFromParent) {
    if (this.get('isDestroyed')) return this; // nothing to do

    // This method is optimized for performance, so the entire sequence of 
    // function calls made from this method work together to achieve the 
    // desired end result.

    // First, destroy any children (but leave their parentView and layers
    // as they were).
    var childViews = this.get('childViews'),
        len = childViews.length, idx, view ;
    for (idx=0; idx<len; ++idx) {
      view = childViews[idx];
    ...
    if (!skipRemoveFromParent) {
      ...
      var layer = this.get('layer'), superlayer;
      if (layer) {
  ...
  */
  ...
  /**
    Returns true if the provided touch is within the boundary.
  */
  touchIsInBoundary: function(touch) {
    console.log("SC.View#touchIsInBoundary()");
    console.log("FIXME: Implement me.");
    return true;
  },
  ...
  */
  init: function() {
    ...
    // Configure child views.  Be sure to clone the child views array first.
...
/** @scope SC.View.prototype */ { // Monkey patch for now.
  ...
  */
  layoutDidChange: function() {
    console.log("SC.View#layoutDidChange()");
