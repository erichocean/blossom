...
/** @private
  ...
  A psurface's only purpose in life is to quickly sync up with the surfaces 
  ...
  the point of view of the surfaces tree_.  A psurface handles these commands 
  ...
  psurfaces that are in the process of being removed.  These psurface are 
  held in the `SC.psurfacesBeingRemoved` hash.

  While bringing a presentation tree in sync with a surfaces tree, the 
  `SC.psurfacesBeingMoved` hash can be used to remember psurfaces that are 
  temporarily detached from the current psurfaces tree, but are known to 
  become attached in the end.
  ...
  - psurface.pop(surface)
  - psurface.next(surface)
  - psurface.end(surface)
...
/** @private */
SC.activePsurface = null; // Mostly a safety check to make sure our callers
...
SC.Psurface.begin = function(surface) {
  ...
  sc_assert(SC.activePsurface === null);
  ...
  return (SC.activePsurface = psurface);
};
...
SC.Psurface.end = function(surface) {
  ...
  sc_assert(SC.activePsurface === psurface);

  SC.activePsurface = null;
  return psurface;
};
