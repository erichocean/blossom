...
/** @class
  ...
  SC.ComboboxWidget is a special type of text field that can display a menu 
  to change its `value` property.  Each menu item has a value assigned to it.  
  ...
  @extends SC.TextFieldWidget
  @since Blossom 1.0
*/
SC.ComboboxWidget = SC.TextFieldWidget.extend({

  /**
    The value of the selected menu item.

    The ComboboxWidget's value will always be the value of the currently
  ...
  */
  ...
  _sc_value: null,
  _sccb_valueDidChange: function() {
    console.log('SC.ComboboxWidget#_sccb_valueDidChange()', SC.guidFor(this));
  ...
  */
  ...
  /**
    ...
    various itemKey properties to tell the ComboboxWidget how to extract the
  ...
  */
  ...
  /**
    ...
    that contain private information used by the ComboboxWidget to render.
  ...
  */
  displayItems: function() {
    var items = this.get('items'), loc = this.get('localize'),
      ...
      item, fetchKeys = SC._sc_checkboxWidgetMenuView_fetchKeys, fetchItem = SC._sc_checkboxWidgetMenuView_fetchItem;
  ...
  render: function(ctx) {
    ...
    ctx.fillStyle = this.get('borderColor');
    ctx.beginPath();
    ctx.moveTo(center, 6);
  ...
  mouseDown: function(evt) {
    ...
    if (evt.hitPoint.x < this.get('bounds').width - 26) {
  ...
  init: function() {
    ...
    menuView = this._sc_menuView = SC.ComboboxWidgetMenuView.create({
      value: this.get('value'),
    ...
    arguments.callee.base.apply(this, arguments);
    this._sccb_valueDidChange();
  }

});

SC.ComboboxWidgetMenuView = SC.View.extend({
  ...
  /**
    ...
    The ComboboxWidgetMenuView's value will always be the value of the 
  ...
  */
  ...
  /**
    The array of items to display.  This can be a simple array of strings,
    objects or hashes.  If you pass objects or hashes, you must also set the
    various itemKey properties to tell the ComboboxWidgetMenuView how to 
  ...
  */
  ...
  /**
    This computed property is generated from the items array based on the
    itemKey properties that you set.  The return value is an array of arrays
    that contain private information used by the ComboboxWidgetMenuView to 
    render.
  ...
  */
  _sc_itemContentDidChange: function() {
    ...
    displayItems.forEach(function(item, idx) {
      ...
      var menuItem = SC.CheckboxMenuItemLayer.create({
  ...
  updateLayout: function() {
    // console.log('SC.ComboboxWidgetMenuView#updateLayout()', SC.guidFor(this));
...
SC._sc_checkboxWidgetMenuView_fetchKeys = function(k) { return this.get(k); };
SC._sc_checkboxWidgetMenuView_fetchItem = function(k) {
  if (!k) return null;
  return this.get ? this.get(k) : this[k];
};

/** @private */
SC.CheckboxMenuItemLayer = SC.Layer.extend(SC.Control, {
  ...
  render: function(ctx) {
    // console.log('SC.CheckboxMenuItemLayer#render()', SC.guidFor(this));
