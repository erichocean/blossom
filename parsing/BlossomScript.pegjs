...
{
  // Any variables or functions defined in here are visible to all semantic 
  // actions below.
}

start
  = head:__ program:Program tail:__ {
    var headAry = [];
    ...
    tail.forEach(function(element) {
      if (typeof element === 'object') {
        switch (element.type) {
          ...
          case "JSLintOptions":
          case "Documentation":
            tailAry.push(element);
          default:
            // console.log('unhandled element.type',  element.type);
        }
      }
    });
    program.elements = headAry.concat(program.elements).concat(tailAry);
    return program;
  }
...
Comment "comment"
  = globals:Globals { return { type: "Globals", value: globals }; }
  / jslint:JSLintOptions { return { type: "JSLintOptions", value: jslint }; }
  / docs:Documentation { return { type: "Documentation", value: docs }; }
  / MultiLineComment
  / SingleLineComment

Globals
  = head:"/*globals" body:(!"*/" SourceCharacter)* tail:"*/" {
      var globals = head + body.map(function(ary) { return ary[1]; }).join('') + tail;
      return globals;
    }

JSLintOptions
  = head:"/*jslint" body:(!"*/" SourceCharacter)* tail:"*/" {
      var jslint = head + body.map(function(ary) { return ary[1]; }).join('') + tail;
      return jslint;
    }

Documentation
  = head:"/**" body:(!"*/" SourceCharacter)* tail:"*/" {
      var doc = head + body.map(function(ary) { return ary[1]; }).join('') + tail;
      return doc;
    }
...
SourceElements
  = head:SourceElement tail:(__ SourceElement)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        var whitespace = tail[i][0];
        whitespace.forEach(function(element) {
          if (typeof element === 'object') {
            switch (element.type) {
              case "Globals":
              case "JSLintOptions":
